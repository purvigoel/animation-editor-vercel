...
import {lbs} from "./lbs_batched.js";
...
export class SMPL{
    ...
    async forward_all(){
        ...
        this.full_pose = full_pose.clone();
    ...
    async forward( ){
        ...
        full_pose = tf.tensor(full_pose.data, full_pose.shape);
        // full_pose = full_pose.arraySync();
        // full_pose = tf.tensor(full_pose[0][10], [ 24, 3, 3]).expandDims(0);
        this.full_pose = full_pose.clone();
        let betas = tf.zeros([1, 10]);

        let mesh, T;
        [this.curr_joints, mesh, this.curr_As, T] = await lbs(betas, full_pose, this.v_template, this.shapedirs, this.posedirs, this.J_regressor, this.parents, this.lbs_weights, false);
        return [this.curr_joints, mesh, this.curr_As, T];
    }
